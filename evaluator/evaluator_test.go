...
func testBooleanObject(t *testing.T, obj object.Object, expected bool, i int) bool {
	...
	if !ok {
		t.Errorf("test case: %v, object is not Boolean. got=%T (%+v)", i, obj, obj)
		return false
	}

	if result.Value != expected {
		t.Errorf("test case: %v, object has wrong value. got=%t, want=%t", i, result.Value, expected)
		return false
	}
	return true
}
...
func testNullObject(t *testing.T, obj object.Object) bool {
	if obj != NULL {
		t.Errorf("object if not NULL. got=%T (%+v)", obj, obj)
		return false
	}
	return true
}
	...
	for _, tt := range tests {
		...
		case string:
			...
			if errObj.Message != expected {
				t.Errorf("wrong error message. expected=%q, go=%q", expected, errObj.Message)
			}
		}
	}
}
